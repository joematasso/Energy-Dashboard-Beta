...
function renderConvoList() {
  ...
  // Render broadcasts section at top if any exist
  let broadcastHtml = '';
  if(CHAT_STATE.broadcasts.length > 0) {
    const unreadCount = CHAT_STATE.broadcasts.filter(b=>!b.read).length;
    const latestBroadcast = CHAT_STATE.broadcasts[0];
    const preview = (latestBroadcast.body || '').substring(0, 50) + ((latestBroadcast.body||'').length > 50 ? '…' : '');
    const time = new Date(latestBroadcast.time).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'});
    broadcastHtml = `<div class="chat-convo-item ${unreadCount>0?'unread':''}" onclick="openBroadcasts()" style="border-left:3px solid var(--amber);background:${unreadCount>0?'rgba(245,158,11,0.08)':'transparent'}">
      <div class="convo-avatar" style="background:var(--amber);color:#000;font-size:16px">📡</div>
      <div class="convo-info"><div class="convo-name" style="font-weight:600">System Broadcasts</div><div class="convo-preview">${preview}</div></div>
      <div class="convo-meta"><span>${time}</span>${unreadCount>0?`<span class="convo-unread">${unreadCount}</span>`:''}</div>
    </div>`;
  }

  if(!CHAT_STATE.conversations.length && !CHAT_STATE.broadcasts.length) {
  ...
  const convoHtml = CHAT_STATE.conversations.map(c => {
  ...
  list.innerHTML = broadcastHtml + convoHtml;
}

function openBroadcasts() {
  CHAT_STATE.activeConvo = null;
  CHAT_STATE.viewingBroadcasts = true;
  CHAT_STATE.showingPicker = false;
  // Mark all broadcasts as read
  CHAT_STATE.broadcasts.forEach(b => b.read = true);
  updateUnreadBadge();
  ...
  try { document.getElementById('chatRenameBtn').style.display = 'none'; } catch(e){}
  try { document.getElementById('chatAddMembersBtn').style.display = 'none'; } catch(e){}
  document.getElementById('chatTitle').textContent = '📡 System Broadcasts';
  const headerAvatar = document.getElementById('chatHeaderAvatar');
  if(headerAvatar) headerAvatar.innerHTML = '<span style="font-size:18px">📡</span>';
  // Render broadcasts as messages
  const container = document.getElementById('chatMessages');
  if(!CHAT_STATE.broadcasts.length) {
    container.innerHTML = '<p style="text-align:center;color:var(--text-muted);padding:20px;font-size:13px">No broadcasts yet.</p>';
  } else {
    container.innerHTML = CHAT_STATE.broadcasts.map(b => {
      const time = new Date(b.time).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'});
      const date = new Date(b.time).toLocaleDateString('en-US',{month:'short',day:'numeric'});
      const urgentStyle = b.isUrgent ? 'border-left:3px solid var(--red);background:rgba(239,68,68,0.08);' : 'border-left:3px solid var(--amber);background:rgba(245,158,11,0.06);';
      return `<div style="padding:12px 14px;margin:6px 0;border-radius:8px;${urgentStyle}">
        <div style="font-weight:600;font-size:13px;margin-bottom:4px;color:${b.isUrgent?'var(--red)':'var(--amber)'}">${b.title}</div>
        <div style="font-size:13px;color:var(--text);line-height:1.5;white-space:pre-wrap">${b.body}</div>
        <div style="font-size:11px;color:var(--text-muted);margin-top:6px">${date} at ${time}</div>
      </div>`;
    }).join('');
  }
  container.scrollTop = 0;
  // Hide input bar for broadcasts (read-only)
  const inputBar = document.getElementById('chatInputBar');
  if(inputBar) inputBar.style.display = 'none';
}

function updateUnreadBadge() {
  const convoUnread = CHAT_STATE.conversations.reduce((s,c)=>s+(c.unread||0),0);
  const broadcastUnread = CHAT_STATE.broadcasts.filter(b=>!b.read).length;
  const total = convoUnread + broadcastUnread;
...
function chatShowList() {
  CHAT_STATE.activeConvo = null;
  CHAT_STATE.showingPicker = false;
  CHAT_STATE.viewingBroadcasts = false;
  ...
  try {
    ...
    if(sock) {
      sock.on('connect', function() {
        // Register this trader so the server can route messages to us
      ...
      sock.on('new_message', function(data) {
        ...
        // Always refresh conversations list to ensure badge is accurate when chat is closed
        if(!CHAT_STATE.open && STATE.trader) {
      ...
      sock.on('admin_broadcast', function(data) {
        ...
        // Route to chat tab instead of notifications
        CHAT_STATE.broadcasts.unshift({
          title: title,
          body: body,
        ...
        updateUnreadBadge();
        // If chat is open, refresh the view
        if (CHAT_STATE.open) {
          if (CHAT_STATE.viewingBroadcasts) {
            openBroadcasts(); // re-render broadcast view
          } else if (!CHAT_STATE.activeConvo) {
            renderConvoList(); // refresh list to show new broadcast indicator
          }
        }
        if (isUrgent) {
          playSound('alert');
          toast('🔴 URGENT BROADCAST: ' + (data.subject || body.slice(0, 60)), 'error');
        } else {
          toast('📡 New broadcast — check your chat', 'info');
        }
      });
