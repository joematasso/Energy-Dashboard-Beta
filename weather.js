...
function toggleWxSection() {
  const section = document.getElementById('ngWxSection');
  const arrow = document.getElementById('wxArrow');
  if (!section) return;
  const isOpen = section.classList.toggle('open');
  if (arrow) arrow.textContent = isOpen ? '▼' : '▶';
  if (isOpen && !STATE.weather) fetchWeather(true);
}
...
async function fetchWeather(force) {
  ...
  try {
    ...
    if (STATE.currentPage === 'ng' || STATE.currentPage === 'weather') renderWeatherPage();
